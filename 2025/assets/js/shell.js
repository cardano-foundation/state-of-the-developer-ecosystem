...
fetch(`${root}/data/survey.json`).then(res => res.json()).then(rawSurvey => {
  ...
  questions.question11.comment = `A median score of 7 with an interquartile range (abbrev. IQR) from 5 to 8 indicates a confident but not complacent developer base: most respondents feel technically capable, yet acknowledge meaningful gaps in their understanding. Compared with 2024, the distribution remains broadly stable, reinforcing the picture of a maturing ecosystem where hands-on experience—rather than formal documentation—drives competence. The lower bound of the IQR suggests that even engaged developers still encounter conceptual or tooling friction, aligning with persistent feedback around documentation and onboarding challenges.`

  questions.question12.comment = `Viewed across experience levels, the Condorcet results reveal a clear fault line between structural scaling priorities and onboarding-oriented concerns. Overall, throughput and documentation emerge as high priorities, but for very different reasons: senior developers converge strongly on throughput and performance-related roadmap items, while beginners distribute their preferences across Layer 2s, primitives, and accessibility, with markedly higher polarization.\n\nNotably, governance consistently ranks low and highly polarizing across all groups, suggesting it is either a background concern or one that only mobilizes a small, committed subset.\n\nCompared with 2024, this sharper divergence by experience signals an ecosystem that is maturing technically while still struggling to present a coherent, experience-level–agnostic roadmap narrative.`
  ...
  questions.question15.comment = `The median satisfaction score of 7, combined with a relatively tight interquartile range (6–8), suggests a broadly shared, cautiously positive sentiment toward the smart contract ecosystem.\n\nCompared with 2024, the narrower spread indicates increasing convergence in developer expectations, likely reflecting stabilization around Aiken and more mature off-chain tooling. However, the absence of a higher median hints that incremental improvements are now expected as baseline, and that future gains in satisfaction will likely depend on advances in testing, debugging, and transaction ergonomics rather than new language introductions alone.`;

  questions.question16.comment = `The Condorcet results distinguish clearly between broad appeal and strong priority consensus. Identity and Authentication emerges as the most widely attractive category, appearing consistently across the top half of rankings, but with little agreement on its exact position—indicating importance without a shared sense of urgency. In contrast, DeFi and Core Infrastructure show a much sharper signal: they concentrate heavily in the top ranks with high popularity scores, reflecting clear consensus that these are first-order priorities rather than merely acceptable ones. Payments sit between these poles, ranking high overall but, like identity, lacking a decisive “must-be-first” signal.`
  ...
  questions.question22.comment = `Across all respondents, the preference structure shows a strong default consensus around JSON over HTTP as the undisputed baseline, while most other protocols compete for relevance based on context rather than principle. Filters clarify this further: professionals and Go-proficient developers elevate gRPC and CBOR over HTTP much higher than the aggregate view, with clearer top-rank concentration, indicating performance and schema-driven concerns in production settings.\n\nIn contrast, hobbyists exhibit flatter rankings across JSON and WebSocket options, reflecting experimentation rather than strong protocol conviction. Familiarity with other ecosystems reinforces this split, with cross-chain developers more willing to trade simplicity for efficiency and stronger interface guarantees—mirroring patterns seen in non-EVM infrastructure-heavy ecosystems.`;

  questions.question23.comment = `CLI usage in 2025 reinforces a layered tooling model: \`cardano-cli\` remains the unavoidable foundation, while purpose-built tools like Aiken and Yaci’s devkit are increasingly adopted to offset its complexity.\n\nCompared to 2024, this suggests not a decline of core tooling, but a growing ecosystem of specialized companions that abstract common workflows such as smart contract development, signing, and testing. The continued presence of tools like tx3—despite maintenance concerns raised elsewhere—highlights persistent gaps in transaction-building ergonomics that newer tools have not yet fully absorbed.`;

  questions.question24.link = `${openQuestions}/it-would-be-nice-if-there-were-a-cli-for.md`;
  questions.question24.comment = `Requests for new CLI tooling in 2025 consistently point toward higher-level, end-to-end workflows rather than additional low-level primitives. Developers are asking for better transaction construction, inspection, and debugging—particularly around Aiken, CBOR, and stateful transaction flows—suggesting that existing CLIs are powerful but fragmented and cognitively demanding.\n\nThe emergence of ideas such as AI-assisted transaction building and governance-facing CLIs (e.g. DRep voting) also signals a shift toward usability and operational ergonomics, echoing trends in other ecosystems where developer tooling increasingly abstracts protocol complexity without sacrificing control.`
  ...
  questions.question25.comment = `Infrastructure preferences in 2025 continue to favor pragmatic, scalable deployment models, with public cloud and third-party services forming the backbone of most production setups.\n\nCompared to 2024, interest in decentralized cloud remains present but secondary, suggesting that ideological alignment alone does not yet outweigh operational maturity and cost efficiency. The coexistence of private and hybrid cloud choices points to a subset of teams with stronger compliance, control, or performance requirements, mirroring broader industry patterns observed in surveys such as Stack Overflow, where public cloud similarly dominates developer deployments.`

  questions.question26.comment = `The preference ordering reveals a two-track consensus around software consumption: language package managers and Docker both attract strong first-rank concentration, indicating they are seen as the default paths for developer-facing and operational use cases respectively. Package managers show broad, stable appeal across the top ranks—suggesting convenience and ecosystem integration—while Docker’s sharper first-rank signal reflects its role as the preferred vehicle for deployment and reproducibility.\n\nIn contrast, source-based builds and pre-compiled binaries exhibit more diffuse rankings, indicating situational relevance rather than clear priority, while Nix remains distinctly polarizing: valued by a committed minority but rarely seen as a general-purpose default.`

  questions.question27.link = `${openQuestions}/what-do-you-think-is-the-greatest-asset-of-cardano-s-developer-ecosystem.md`;
  questions.question27.comment = `Responses consistently frame Cardano’s greatest asset as its people rather than any single technology, highlighting a culture of collaboration, rigor, and long-term commitment that differentiates the ecosystem from more transactional developer communities.\n\nCompared with 2024, there is a clearer recognition that tools like Aiken and high-quality infrastructure succeed largely because they are embedded in an open, values-driven culture of knowledge sharing and patience. The combination of strong technical foundations (eUTxO, FP, determinism) with a resilient, talent-dense community emerges as Cardano’s distinctive advantage—one that compounds over time, even as short-term developer experience challenges persist.`;
  ...
  questions.question28.comment = `The dominant pain points expressed in 2025 cluster around developer experience rather than protocol capability, with fragmentation of tooling, weak off-chain/on-chain integration, and the absence of mature transaction-building abstractions recurring across responses.Compared to 2024, the emphasis shifts from documentation alone toward a broader critique of ecosystem cohesion: duplicated efforts, unclear “blessed paths,” and insufficiently maintained libraries amplify the already steep eUTxO learning curve.\n\nIn contrast to ecosystems like Ethereum—where complexity is often hidden behind opinionated frameworks—Cardano developers still shoulder much of the architectural burden directly, slowing onboarding and experimentation despite strong underlying primitives.`;
  ...
  questions.question29.comment = `The support landscape in 2025 remains strongly community- and peer-driven, with real-time and semi-synchronous channels continuing to dominate over traditional Q&A platforms. Compared with 2024, reliance on GitHub and Discord appears even more entrenched, reflecting a preference for context-rich, project-adjacent discussions rather than generic forums.\n\nThe growing, though still secondary, use of AI/LLMs suggests developers increasingly supplement human help with automated assistance, while the relatively low use of StackExchange highlights a persistent gap in structured, archival knowledge for Cardano-specific issues.`;

  questions.question30.comment = `The results suggest that Cardano developers continue to rely heavily on primary sources—notably source code and project-specific documentation—rather than purely narrative or marketing-driven material. This reinforces a long-standing pattern seen in 2024: trust and authority are still derived from proximity to implementation and specifications.\n\nThe relatively strong presence of informal channels (direct peers) alongside official portals highlights the importance of social knowledge transfer, while the modest but growing use of AI/LLMs signals an emerging shift in how developers triangulate information rather than a replacement of canonical sources.`;

  questions.question31.comment = `A median score of 7 with a notably tight interquartile range (6–7) suggests moderate satisfaction coupled with broad agreement, indicating that most developers converge on a “good enough, but not great” assessment of technical answers. Compared with 2024, this stability reinforces the idea that incremental improvements have not yet shifted overall perception, despite increased reliance on community channels and source code. The narrow spread also implies that frustrations around documentation quality and answer depth are widely shared rather than isolated to specific subgroups.`

  questions.question32.comment = `Engagement with Cardano Improvement Proposals appears notably deep compared with most open-source ecosystems, where governance artifacts are often read but rarely interacted with directly. The sizeable share of respondents involved in CIP discussions—and especially authorship—suggests that Cardano’s governance processes are not only visible but actively participatory among developers.\n\nCompared with 2024, this reinforces the picture of a technically mature community where protocol evolution is perceived as accessible rather than opaque. The very small group unfamiliar with CIPs indicates that standards and technical solutions literacy is now close to baseline knowledge within the developer ecosystem.`;

  questions.question33.comment = `The responses confirm that English remains the de facto lingua franca for Cardano’s technical ecosystem, even among an internationally diverse developer base. However, the consistent presence of German, Spanish, Portuguese, and Japanese—mirroring patterns seen in prior years—suggests sustained demand for targeted, high-quality translations rather than broad multilingual coverage.`;

  questions.question34.comment = `Attendance data suggests that demand for in-person, developer-focused events currently exceeds capacity: a large share of respondents report wanting to attend the Buidler Fest but being unable to do so. This gap between interest and participation mirrors feedback from previous years and indicates sustained appetite for hands-on, technically oriented gatherings.\n\nCompared with 2024, the signal remains consistent—Buidler Fest appears well-positioned as a high-value event, with constraints likely stemming from logistics or selectivity rather than lack of relevance.`;

  questions.question35.comment = `The very high median of 9 with a tightly clustered interquartile range (8–10) indicates exceptionally strong and consistent satisfaction among attendees, with little evidence of polarized experiences. Compared with broader ecosystem sentiment questions, this stands out as one of the few areas showing near-unanimous positivity, reinforcing the value of focused, in-person, developer-first formats. The elevated floor (minimum 7) suggests that even critical participants found the event worthwhile, strengthening the case for continuing—and carefully scaling—this model in future editions.`
  ...
  questions.question36.comment = `Feedback on improving the Cardano Buidler Fest converges on deepening its “builder-first” identity rather than broadening its scope. Respondents favor more time for hands-on hacking, unstructured collaboration, and substantive infrastructure-focused discussions over additional talks or product showcases.\n\nThe emphasis on maintaining a small, selective, and recurring format suggests the event is valued as a high-signal space for serious contributors, with refinements aimed at maximizing peer interaction and technical depth rather than increasing scale.`;
